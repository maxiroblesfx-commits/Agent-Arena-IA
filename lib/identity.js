...
/** Scout — semilla de handles a mirar en FOMO.
 *  No trae wallets ni PnL: ver el bloque PURGADO más abajo. Las wallets se
 *  resuelven contra la API real (lib/resolve.js) o se pegan a mano.
 */
...
/* Handles VISTOS en datos reales: aparecieron en el tape capturado el
 * 2026-09-04 (/feed/tradingActivity), así que existen. Nada más que eso: que
 * el handle sea real no dice absolutamente nada sobre si el trader es bueno.
 * econoar existe y puntúa 6/100. */
const OBSERVADOS = ["unipcs", "himgajria", "DumbCrayonEater", "soby0x"];

/* Handles heredados del catálogo que se purgó en 2026-09-03. Los datos que
 * los acompañaban eran inventados; los handles se conservaron por si servían
 * de semilla, pero NINGUNO fue visto todavía en datos reales. No tratarlos
 * como existentes hasta comprobarlo — la lista se llamaba "VERIFIED" y esa
 * mentira de nombre es la que dejó pasar el desastre anterior. */
const SIN_COMPROBAR = [
  ...
  "vancute1112", "thejefeee",
];

const CATALOGO = [
  ECONOAR,
  ...OBSERVADOS.map((h) => ({ ...semilla(h), visto: "tape 2026-09-04" })),
  ...SIN_COMPROBAR.map((h) => semilla(h)),
];
...
for (const t of [...CATALOGO, ...JUNK]) index(t);
...
module.exports = {
  ...
  isSolanaAddress,
  isEvmAddress,
  // La watchlist real: econoar + los cuatro que aparecieron en el tape.
  // Los handles sin comprobar quedan fuera del arranque a propósito.
  SEED_WATCH: ["econoar", ...OBSERVADOS],
};
